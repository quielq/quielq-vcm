...
step 7 below) currently combines **40,442 rows** from 4 sources:

| Label coverage | Source | Rows |
|---|---|---:|
| 13 of 19 labels | Real audio (SLURP) | 19,712 |
| All 19 labels | Synthetic, QA-filtered audio (class-shared "Option B" dataset) | 17,658 |
| `unknown_background` only | Real noise, chopped into clips (Google Speech Commands v2) | 600 |
| 4 lighting labels only (LIGHT_ON/OFF, BRIGHTNESS, COLOR) | Real audio, text-classified (Snips SLU) | 2,472 |
| 6 labels with zero real coverage | PAUSE, STOP, NEXT, CALL, TIMER, TEMPERATURE — synthetic-only, checked against every candidate source found (see step 6) and none cover them | — |
...
lighting — whichever of steps 3/4/5/6 you've actually run; missing ones
are skipped with a note, not an error) into one
...
## 8. FSC coverage check (needs your own file — not automated here)

Unlike SLURP, Fluent Speech Commands isn't freely downloadable without a
Kaggle account (or a Fluent.ai license request) — tried a few more
unauthenticated routes (Fluent.ai's own site, alternate HF mirrors),
none worked. If you have `train_data.csv` (columns: `path, speakerId,
...
was, once someone has the actual file. This remains the highest-value
open dataset gap: Section 9's original research claims FSC covers
thermostat control and media commands, which could help several of the
6 currently-synthetic-only labels if confirmed.
...
python -m pytest
```
57 tests total; the dataset-specific ones are `tests/test_dataset_*.py`,
