...
step 9 below) currently combines **62,405 rows** from 5 sources (a 6th,
...
lighting, FSC, real recordings — whichever of steps 3/4/5/6/7/8 you've
actually run; missing ones are skipped with a note, not an error) into
one `data/dataset_manifest.csv`, in the common schema every source
...
taxonomy. `scripts/build_manifest.py` (step 9) is the concrete example
