...
## Draft Label Taxonomy Review (`Dataset_Schema.xlsx`)

Quiel's own in-progress schema, built ahead of class-wide taxonomy finalization, structured more cleanly than the ten broad categories this document started with: **13 fixed-phrase intents** (PLAY_MUSIC, WEATHER, TIME, LIGHT_ON, LIGHT_OFF, PAUSE, STOP, NEXT, VOLUME_UP, VOLUME_DOWN, CALL, MESSAGE, LIST_REMINDERS) plus **6 slotted intents** (TIMER{duration}, ALARM{time}, TEMPERATURE{degrees}, BRIGHTNESS{percent}, COLOR{color}, CREATE_REMINDER{task}). This explicitly answers Section 7's open question on whether slot-filling is in scope, this schema says yes, with a clean fixed-vs-slotted split, which is a stronger design than treating every command as a flat label. Three sheets (Option A/B/C) offer the same 19 intents at increasing richness, 2 vs. 3 phrasing variations per intent and 2 vs. 3 example slot values per slotted intent, a reasonable way to scale up phrase coverage without changing the taxonomy itself.

**A real bug to fix before this schema is used to generate anything**: the ALARM intent's slot values aren't stored as plain text the way every other slot is ("10 seconds," "18 degrees," "Red"). Excel auto-detected "6 AM" and "8 AM" as literal time values and silently converted them to internal time objects (confirmed directly from the file: stored as Python `datetime.time` objects with a `h:mm am/pm` display format, not strings). Worse, the third value in Options B and C is stored as `21:00` (9:00 PM), not the "8 PM" it appears to show, an internal inconsistency between the stored value and whatever displayed it as "8 PM" elsewhere. If a data-generation script reads this file with pandas or openpyxl as-is, the ALARM slot will come back as a time object or datetime string instead of the same "6 AM"-style text every other slot produces, and will silently carry the wrong hour for that third value. Fix at the source: reformat that column as Text and retype the values as literal strings ("6 AM", "8 AM", whatever the third value is actually meant to be) before this schema feeds any generation pipeline.
