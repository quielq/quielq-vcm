...
- **How this build is using classmates' contributions, Sept 17 2026**: this project's dataset pipeline is explicitly built *on top of* work already shared in the class, not developed in isolation. Concretely: the working taxonomy is Mark Andrian Macalalad's fixed-vs-slotted schema, adopted as-is (see the Draft Label Taxonomy Review section below), with a plain-CSV export now maintained alongside it for safer downstream tooling; the plan for synthetic data generation, if a real coverage gap remains after checking real sources, is to extend Mark Andrian's speaker-disjoint Chatterbox TTS methodology rather than invent a separate approach, since it already targets the Filipino-accent gap with real rigor; the synthetic-data QA step being built generalizes Anthony Navarez's `simple-audio-transcriber` transcribe-and-compare pattern so it can screen any generator's output, not just his own; and before generating any new synthetic audio at all, Cherry Magdaong's `SynTTS-Commands-Official` find is being checked for existing coverage, to avoid the class duplicating effort.
- **SLURP coverage against Mark Andrian's taxonomy, verified Sept 17 2026** (downloaded the real train/devel/test.jsonl from github.com/pswietojanski/slurp, 16,521 sentences / 72,396 recordings total, matching this document's earlier ~72,000 figure exactly): of the 19 labels, **6 have zero SLURP coverage** — PAUSE, STOP, NEXT (SLURP has no media-transport-control domain at all, only `play_*` intents), CALL (no calls/phone domain in any of SLURP's 18 scenarios), TIMER (SLURP's `alarm` scenario only covers wake-alarms, no countdown-timer intent), and TEMPERATURE (no thermostat intent within its `iot` scenario, despite that scenario covering lights). The remaining 13 have real matches, strongest being PLAY_MUSIC (911 sentences / 3,883 recordings), WEATHER (834/3,412), and general_quirky-adjacent TIME via `datetime_query` (490/2,558, though this mixes in date queries). MESSAGE only has a weak proxy (`email_sendemail`, 523/1,925), not a real phone/SMS-message match. This sharpens the earlier qualitative FSC-gap note into a precise picture for both real sources: between SLURP and FSC, **calls, timers, and thermostat control have no real-dataset coverage at all** in either, real recordings (or synthetic, if that gap can't be closed with real class recordings) are the only path for those three labels specifically.
...
**A spreadsheet-format quirk worth knowing about before scripting against the live sheet**: Google Sheets/Excel auto-detect and internally store time-like text (e.g. "6 AM", "8 AM") as an actual time type rather than plain text, common, expected spreadsheet behavior, not something specific to this file. Practically: a script reading the ALARM column directly out of the live sheet via pandas/openpyxl will get a `datetime.time` value instead of a string, unlike every other slotted intent's example values. To sidestep this entirely rather than working around it inline, this project maintains a plain-CSV export of the full taxonomy (`data/dataset_schema/dataset_schema.csv`, generated from `src/vcm/dataset/sources/dataset_schema.py`), where every value is a literal string by construction, safe to load anywhere without touching Sheets formatting at all.
...
**How this project is using it**: `quielq-vcm`'s dataset pipeline (see the repo's own README/commits) adopts this taxonomy as its working schema rather than the ten broad categories this document started with, and builds a loader against it directly, decoupled from audio so it's ready to consume whichever recordings the class collects. The taxonomy itself isn't being modified, only the CSV export and tooling built around it.
